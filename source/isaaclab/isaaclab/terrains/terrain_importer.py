...
class TerrainImporter:
    ...
    def import_ground_plane(self, name: str, size: tuple[float, float] = (2.0e6, 2.0e6)):
        ...
        if self.cfg.visual_material is not None:
            ...
            if "diffuse_color" in material:
                ...
                omni.log.warn("Visual material specified for ground plane but no diffuse color found." " Using default color: (0.0, 0.0, 0.0)")
    ...
    def import_mesh(self, name: str, mesh: trimesh.Trimesh):
        ...
        # import the mesh
        create_prim_from_mesh(prim_path, mesh, visual_material=self.cfg.visual_material, physics_material=self.cfg.physics_material)
    ...
    def _compute_env_origins_curriculum(self, num_envs: int, origins: torch.Tensor) -> torch.Tensor:
        ...
        self.terrain_types = torch.div(torch.arange(num_envs, device=self.device), (num_envs / num_cols), rounding_mode="floor").to(torch.long)
    ...
    def warp_meshes(self):
        ...
        """
        omni.log.warn(
            "The `warp_meshes` attribute is deprecated. It is no longer stored inside the `TerrainImporter` class." " Returning an empty dictionary."
        )
        return {}
    ...
    def meshes(self) -> dict[str, trimesh.Trimesh]:
        ...
        """
        omni.log.warn(
            "The `meshes` attribute is deprecated. It is no longer stored inside the `TerrainImporter` class." " Returning an empty dictionary."
        )
        return {}
