...
class SealGoToXYZEnvCfg(SealEnvCfg):
    # Here you can define task-specific configurations
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.observation_space_task = (
            3  # Task-specific observations. For GoToXYZ task, we need to observe the relative desired position in the body frame, X, Y, Z
        )
...
class SealGoToXYZEnv(SealEnv):
    cfg: SealGoToXYZEnvCfg
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        action_diff = torch.norm(self._continuous_cmd - self._prev_continuous_cmd, dim=1)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        goal_reached = torch.logical_and(self.distance_to_goal > 0.0, self.distance_to_goal < 0.3)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._desired_pos_w[env_ids, 2] = torch.zeros_like(self._desired_pos_w[env_ids, 2]).uniform_(0.0, 3.0)

        default_root_state = self._robot.data.default_root_state[env_ids]
        # Randomize the robot's initial position
        random_offsets = torch.zeros_like(self._terrain.env_origins[env_ids])
        random_offsets[:, 0] = torch.FloatTensor(len(env_ids)).uniform_(-2.0, 2.0)  # Randomize x position
        random_offsets[:, 1] = torch.FloatTensor(len(env_ids)).uniform_(-2.0, 2.0)  # Randomize y position
        random_offsets[:, 2] = torch.FloatTensor(len(env_ids)).uniform_(-6.0, -4.0)  # Randomize z position
