...
class Aerodynamics:
    ...
    def ComputeAeroDampingMatrix(self, vel):
        """
        ...
        # print("vel: ", vel)
        lin_damp = (
            self.linear_damping + self.cfg.offset_linear_damping - (self.linear_damping_forward_speed + self.cfg.offset_lin_forward_damping_speed)
        )
    ...
    def ComputeAerodynamicsEffects(self, quaternions, world_vel, positions_z):
        # Debug
        ...
        # Debug
        # print(f"local_lin_velocities: {self.local_lin_velocities}")
        self.local_ang_velocities = quat_rotate_inverse(quaternions, world_vel[:, 3:])
        # Debug
        # print(f"local_ang_velocities: {self.local_ang_velocities}")
        self.local_velocities = torch.hstack([self.local_lin_velocities, self.local_ang_velocities])
        # Debug
        # print(f"local_velocities: {self.local_velocities}")
        # Update damping matrix
        damping_matrix = self.ComputeAeroDampingMatrix(self.local_velocities)
        # Debug
        # print(f"damping_matrix: {damping_matrix}")
        ...
        # aerodynamics only applies when the robot is in the air(overwater).
        mask = (positions_z > 0.0).float().unsqueeze(1)
        self.drag = self.drag * mask
